...
$result = $conn->query("SELECT * FROM csrs WHERE user_id = $user_id");

$pdf = new FPDF();
$pdf->AddPage();
$pdf->SetFont('Helvetica', 'B', 16);
$pdf->Cell(0, 10, 'My CSR Report', 0, 1, 'C');
$pdf->Ln(10);

$pdf->SetFont('Helvetica', '', 12);
while ($row = $result->fetch_assoc()) {
    $pdf->Cell(0, 10, "CSR ID: " . $row['id'], 0, 1);
...
$pdf->Output('D', 'My_CSR_Report.pdf');
?>
